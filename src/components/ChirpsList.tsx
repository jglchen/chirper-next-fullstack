...
import validateChirpSubmitClient from '@/validate/chirp-submit-client';
...
const ChirpsList = () => {
    ...
    const submitMessage = async (event: FormEvent) => {
        ...
        const validateResult = validateChirpSubmitClient(inputObj);
        if (!validateResult.valid){
            setErrors(validateResult.errorMsg as MsgErrorsType);
            return; 
        }
        ...
        await axios
            ...
            .then(response => {
                ...
                axios
                .post('/api/chirpcreated-notification', {
                    chirp: response.data.chirp,
                    userName: user.name,
                    hosturl:  getHostURL()
                })
                setMessageData('');
    ...
    const initLoadChirps = () => {
        axios
            ...
            .then((response) => {
                ...
                setChirps(response.data.chirps);
            });
    };
    ...
    const loadNextPageChirps = () => {
        ...
        setPage(nextPage);
        console.log('Next',`/api/chirps?p=${nextPage}`);
        axios
            .get(`/api/chirps?p=${nextPage}`)
            .then((response) => {
                setChirps((prevChirps) => {
                    const addedChirps = response.data.chirps;
